...
function set_tinymce_selector_for_editor(editor_id) {
  return tinymce.init({
    ...
    toolbar:
      "undo redo | bold italic underline |fontselect fontsizeselect  |  alignleft aligncenter alignright alignjustify | forecolor backcolor  |ltr rtl",
  });
}
...
function WhatchInteraktionenLeftfunktion(interId, value, CheckId, interKat) {
  $(interId).on("click", function () {
    ...
    if (ScenarioCounter <= 0)
      errorMessage(
        "Fügen Sie bitte ein Szenario hinzu und laden Sie ein Bild in den Arbeitsbereich !"
      );
    else if ($("#main_full" + last_scenario_active).is(":visible"))
      errorMessage("Laden Sie bitte ein Bild in den Arbeitsbereich !");
      ...
      WatchSelectBewertungsmodus(icon_id);
...
/*---------------------------------------------------------------- */
function WatchSelectBewertungsmodus(id) {
  $("#selectBewertungsmodus" + id).on("click", function () {
    const punkte = document.createElement("input");
    punkte.className = "BewertInteraktion";
    punkte.id = "punkteB" + id;
    punkte.type = "number";
    punkte.value = 1;

    const prozent = document.createElement("input");
    prozent.className = "BewertInteraktion";
    prozent.id = "prozentB" + id;
    prozent.type = "number";
    prozent.min = 0;
    prozent.max = 1;
    prozent.step = 0.000001;
    prozent.value = 0.01;

    const div = document.createElement("div");
    div.innerHTML = `
       <h2 style="color:#4b5af3 ">Bewertungsmodus</h2>
       <div class = "line"></div>
         ...
         <br> <input  id = "${
           "punkte" + id
         }" type="checkbox" style = "margin-left:-70%"><label style="font-size:24px ; color:#70717a">Punkte</label><br>
          <br><input  id = "${
            "prozent" + id
          }" type="checkbox" style = "margin-left:-67%"><label style="font-size:24px; color:#70717a">Prozent</label><br>
   
       </div>
        </div>
        `;

    swal({ content: div }).then(function (e) {
      if (
        e &&
        ($("#prozentB" + id).is(":visible") ||
          $("#punkteB" + id).is(":visible"))
      ) {
        $("#selectBewertungsmodus" + id).hide(500);
      }
    });
    $("#punkte" + id).on("change", function () {
      $(this)
        .siblings("#prozent" + id)
        .prop("checked", false);
      $("#selectBewertungsmodus" + id).hide(500);
      $("#prozentB" + id).remove();
      $("#punkteB" + id).show(540);
      $("#UDelete" + id).before(punkte);
    });

    $("#prozent" + id).on("change", function () {
      $(this)
        .siblings("#punkte" + id)
        .prop("checked", false);
      $("#selectBewertungsmodus" + id).hide(500);
      $("#punkteB" + id).remove();
      $("#prozentB" + id).show(540);
      $("#UDelete" + id).before(prozent);
    });
  });
}
/*---------------------------------------------------------------- */
...
function WatchCanvas(context, canvas, check) {
  ...
  $(canvas).on("click", function (Event) {
    if (
      ...
      errorMessage("Wählen Sie bitte eine Interaktionen !");
    }
  });
}
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
...
addButton.addEventListener("click", function () {
 ...
 (if repeated pop-up will show up
  ...
  easier to distinct between them after submitting  */
      if (notRepeated && inputid.value.trim() != "") {
        input.type = "checkbox";
